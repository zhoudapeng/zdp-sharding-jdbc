...
交易库分成2个shard，每个shard中1个主库，2个从库，每个库中2张表 
...
```
### 此框架的不足&展望
1. 对事务的支持，由于拦截器是拦截在DAO方法上，所以如果不在同一个事务中，则每次DAO操作都会切换数据源，有可能由于数据库同步延迟导致刚写入的数据查询的时候查不到 
   如果在同一个事务，但第一个操作是读操作，则后面的写操作也会走读库，会造成写数据失败，这种问题可以在查询的时候显示加@Write注解实现，或者是在更高层级，比如
   Controller层或者Service层上加@Read,@Write注解显示告诉框架此操作是读操作还是写操作
2. 数据源配置的问题，由于数据源的配置都是配置在配置文件中，随着分片的增多，配置文件会越来越麻烦，越来越容易出错，而且一旦涉及到数据库迁移就会非常麻烦， 
   所以后期的想法是将配置维护在集中式配置系统中，比如携程的Apollo，百度的disconf等，这样的话可以实时获取到最新配置，且无需修改配置文件然后重新发布服务
